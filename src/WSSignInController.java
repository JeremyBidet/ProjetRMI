...
public class WSSignInController {
	...
	public void initialize() {
		...
		this.signIn.setOnAction(value -> {
			try {
				...
				String token = _MainWSClient.auth.login(login.getText(), Utils.sha1(password.getText()));
				WebGuiStage wgstage = new WebGuiStage(token, login.getText());
		...
		this.signUp.setOnAction(value -> {
			try {
				WSSignUpStage wssustage = new WSSignUpStage();
				wssustage.show();
				this.signIn.getScene().getWindow().hide();
			} catch (RemoteException e) {
				javax.swing.JOptionPane.showMessageDialog(null,"Connection issue...\nPlease restart application.");
